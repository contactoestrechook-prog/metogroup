...
// ════════════════════════════════════════════════════════
//  SYNC: Supabase examen_resultados → localStorage bpc_exam_resultados
//  + Alertas en el sistema cuando llegan resultados nuevos
// ════════════════════════════════════════════════════════
(function(){

  const SB_URL = 'https://smcghyecpzzimadtuern.supabase.co';
  const SB_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNtY2doeWVjcHp6aW1hZHR1ZXJuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIzODYwMTksImV4cCI6MjA4Nzk2MjAxOX0.u9OgCRMPFnuNLq4UinR1idpRykVF_VKPwIvFXgttVOw';
  const LS_KEY  = 'bpc_exam_resultados';
  const VISTO_KEY = 'bpc_exam_resultados_vistos';

  async function sincronizarExamenes(){
    try{
      const res = await fetch(SB_URL + '/rest/v1/examen_resultados?order=fecha.desc&limit=500', {
      ...
      // Guardar en localStorage para que el mapa los lea
      localStorage.setItem(LS_KEY, JSON.stringify(data));

      // Detectar nuevos resultados (no vistos aún)
      const vistos = JSON.parse(localStorage.getItem(VISTO_KEY) || '[]');
      const nuevos = data.filter(r => r.id && !vistos.includes(r.id));

      if(nuevos.length > 0){
        // Marcar como vistos
        const nuevosIds = nuevos.map(r => r.id);
        localStorage.setItem(VISTO_KEY, JSON.stringify([...vistos, ...nuevosIds]));

        // Agregar alertas BPC para cada resultado nuevo
        const alertas = JSON.parse(localStorage.getItem('bpc_alertas_admin') || '[]');
        nuevos.forEach(r => {
          const tipoLabel = r.tipo === 'diagnostico_gerente' ? '📊 Gerente'
                          : r.tipo === 'diagnostico_dueno'   ? '👑 Dueño'
                          : '👤 Vendedor';
          alertas.push({
            id: 'exam_' + r.id,
            tipo: 'examen_recibido',
            titulo: `Examen recibido — ${tipoLabel}`,
            mensaje: `${r.nombre} (${r.empresa||'—'}) completó su examen. Score: ${r.score}% · Nivel: ${r.nivel}`,
            auditoria_id: r.auditoria_id,
            fecha: r.fecha || new Date().toISOString(),
            leida: false,
          });
        });
        localStorage.setItem('bpc_alertas_admin', JSON.stringify(alertas));

        // Actualizar badge
        if(typeof updateBPCBadge === 'function') updateBPCBadge();

        // Toast informativo
        if(typeof toast === 'function'){
          const n = nuevos.length;
          toast(`📋 ${n} examen${n>1?'es':''} nuevo${n>1?'s':''} recibido${n>1?'s':''} — revisá el mapa de auditoría`);
        }

        // Si el mapa está abierto, refrescarlo
        if(typeof renderAuditorias === 'function'){
          const pageAud = document.getElementById('page-auditorias');
          if(pageAud && !pageAud.classList.contains('hidden')){
            renderAuditorias();
          }
        }
      }
      ...
      console.warn('[patch-examenes] sync resultados falló:', e);
    }
  }

  // Sincronizar al cargar y cada 2 minutos
  ...
  whenReady2(function(){
    // Primera sync a los 3 segundos de carga (esperar que el sistema inicialice)
    setTimeout(sincronizarExamenes, 3000);
    // Sync periódica cada 2 minutos
    setInterval(sincronizarExamenes, 120000);
    // Exponer para llamar manualmente si hace falta
    window.sincronizarExamenes = sincronizarExamenes;
  });
