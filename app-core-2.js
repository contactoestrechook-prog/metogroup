...
function abrirResultadoEntrevista(id){
  ...
  document.getElementById('ent-proximo-paso').value=e.proximoPaso||'';
...
function saveEntrevistaResultado(){
  ...
  const proximoPaso=document.getElementById('ent-proximo-paso').value;
  ...
  if(!interesado){toast('⚠️ Indicá si el cliente está interesado');return;}

  const entrevistas=S.get('crm_entrevistas')||[];
  const idx=entrevistas.findIndex(e=>e.id===id);
  if(idx===-1)return;

  entrevistas[idx]={...entrevistas[idx],fechaRealizada,interesado,resultado,proximoPaso,observaciones};
  S.set('crm_entrevistas',entrevistas);

  // Actualizar etapa en pipeline según resultado
  const emp=entrevistas[idx].empresa;
  if(emp){
    const empresas=S.get('crm_bases_datos')||[];
    const eIdx=empresas.findIndex(e=>e.empresa===emp);
    if(eIdx>-1){
      let nuevaEtapa=empresas[eIdx].etapaPipeline;
      if(interesado.includes('interesado')&&!interesado.includes('No')){
        if(nuevaEtapa==='prospecto'||nuevaEtapa==='contactado')nuevaEtapa='propuesta';
      } else if(interesado==='No interesado'){
        // No movemos automáticamente — el dueño decide
      }
      if(nuevaEtapa!==empresas[eIdx].etapaPipeline){
        empresas[eIdx].etapaPipeline=nuevaEtapa;
        try{let h=JSON.parse(empresas[eIdx].historial||'[]');h.push({fecha:todayStr(),tipo:'pipeline',detalle:`Entrevista realizada — ${interesado} → etapa: ${PIPELINE_ETAPAS.find(e=>e.key===nuevaEtapa)?.label||nuevaEtapa}`,usuario:currentUser?.nombre||''});empresas[eIdx].historial=JSON.stringify(h);}catch(ex){}
        S.set('crm_bases_datos',empresas);
      }
    }
  }
  ...
  toast('✅ Resultado de entrevista guardado');
  renderDashboard();
}
...
function maContexto(vend, vendId){
  ...
  return `Sos MetoAsist, el coach de ventas personal de MetoGroup. Conocés a fondo a ${vend.nombre} porque tenés acceso a todos sus datos reales. Hablás como un mentor cercano: directo, motivador, humano. Usás el voseo rioplatense. Nunca sos genérico — siempre hacés referencia a sus números reales. Recordás TODO lo que hablaron antes (tenés el historial completo). Sos entusiasta pero honesto.

=== PERFIL DE ${vend.nombre.toUpperCase()} ===
...
Llamadas: ${mLL} | Dueños: ${mDU} | Entrevistas: ${mAG} | Cierres: ${mCI}
Tasa de contacto: ${tasaCont}% | Tasa agendado: ${tasaAg}% | Conversión total: ${tasaConv}%
Días activos: ${diasLogs} | Auditorías activas: ${audActivas} | Completadas: ${audComp}
${obj?`Objetivo de cierres: ${obj.cerradas} → va en ${pCI}% del objetivo`:'Sin objetivos configurados'}

=== ESTA SEMANA ===
Llamadas: ${sLL} | Cierres: ${sCI}

=== SEGUIMIENTOS ===
Pendientes: ${segs.length} | Vencidos: ${segsVenc}

${premioMes?`=== PREMIO EN JUEGO ===\n${premioMes.nombre} — ${premioMes.desc}\n`:''}
${notasActivas.length?`=== NOTAS/AVISOS PENDIENTES ===\n${notasActivas.slice(0,3).map(n=>n.titulo+(n.fecha?' ('+fmtD(n.fecha)+')':'')).join(', ')}\n`:''}
REGLAS: Respondé en español rioplatense. Sin asteriscos ni markdown. Sé concreto. Máximo 5 oraciones salvo que te pidan algo más extenso como un script o plan. Si alguien te saluda o hace una pregunta corta, respondé corto. Recordás perfectamente todo lo que hablaron.`;
}
...
#mcl-panel{position:fixed;bottom:24px;right:24px;z-index:1101;width:380px;height:560px;max-height:80vh;display:none;flex-direction:column;border-radius:20px;overflow:hidden;background:linear-gradient(170deg,var(--surface),var(--surface2));border:1px solid var(--border);box-shadow:0 20px 60px rgba(0,0,0,0.4),0 0 0 1px rgba(200,168,74,0.08);transition:all 0.3s cubic-bezier(.16,1,.3,1)}
#mcl-panel.open{display:flex}
.mcl-header{padding:16px 18px;display:flex;align-items:center;gap:12px;border-bottom:1px solid var(--border);background:linear-gradient(135deg,rgba(200,168,74,0.06),transparent)}
...
.mcl-input-area{padding:10px 14px;border-top:1px solid var(--border);display:flex;gap:8px;align-items:flex-end}
.mcl-input{flex:1;background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:9px 14px;color:var(--text);font-size:13px;resize:none;outline:none;font-family:'DM Mono',monospace;min-height:38px;max-height:100px;transition:border-color 0.2s}
          ...
          ${items.sort((a,b)=>(a.fecha||'9999').localeCompare(b.fecha||'9999')).map(s=>{
            ...
            return`<tr style="border-bottom:1px solid rgba(212,175,55,0.07);${venc?'background:rgba(239,68,68,0.04)':''}">
              ...
              <td style="padding:10px 10px;text-align:center;white-space:nowrap">
                <button onclick="convertirSeguimientoEnVenta(${s.id})" title="Convertir en venta" style="background:linear-gradient(135deg,rgba(200,168,74,0.15),rgba(212,175,55,0.08));border:1px solid rgba(200,168,74,0.35);border-radius:6px;padding:4px 8px;cursor:pointer;color:var(--accent3);font-size:11px;font-weight:700">🏆</button>
  ...
  if(linkActivo){
    ...
    const cargarWidget=()=>{
      if(window.Calendly){
        ...
        // Ocultar loader luego de un momento
        setTimeout(()=>{const l=document.getElementById('cly-loading');if(l)l.style.display='none';},2000);
      }
    };
...
(async function(){
  ...
  // examen_token ya redirigió — por seguridad bloquear init
  if(_initParams.get('examen_token')){
  ...
  if(sess){
    try{
      ...
      // ── Cliente portal (id empieza con 'cli_') — no está en tabla usuarios ──
      if(sd.rol === 'cliente' || String(sd.id||'').startsWith('cli_')){
  ...
  <div class="modal-body">
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label>📌 Acción siguiente</label>
        <select id="bdc-accion" onchange="document.getElementById('bdc-seg-fields').style.display=this.value==='seguimiento'?'block':'none';document.getElementById('bdc-venta-fields').style.display=this.value==='venta'?'block':'none'">
          <option value="">— Ninguna —</option>
          <option value="seguimiento">📅 Crear seguimiento (re-llamar)</option>
          <option value="venta">🏆 ¡Registrar como VENTA!</option>
        </select>
      </div>
    </div>
    ...
    <div id="bdc-venta-fields" style="display:none;background:rgba(200,168,74,0.06);border:1px solid rgba(200,168,74,0.2);border-radius:10px;padding:14px;margin-top:8px">
      <div style="font-size:11px;font-weight:600;color:var(--accent3);margin-bottom:8px">🏆 Datos de la venta</div>
      <div style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25);border-radius:8px;padding:10px;font-size:11px;color:#f59e0b;margin-bottom:10px">
        🔔 Se enviará alerta a <strong>Administración</strong> para procesar la venta
      </div>
      <div class="form-row">
        <div class="form-group"><label>Servicio vendido</label>
          <select id="bdc-venta-servicio">
            ...
            <option value="Implementación ISO 72001">Implementación ISO 72001</option>
          </select>
        </div>
        <div class="form-group"><label>Monto USD</label><input type="number" id="bdc-venta-monto" value="2000" min="0"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>CUIT</label><input id="bdc-venta-cuit" value="${e.cuit||''}" placeholder="CUIT de la empresa"></div>
        <div class="form-group"><label>Rubro</label><input id="bdc-venta-rubro" value="${e.rubro||''}" placeholder="Rubro"></div>
      </div>
    </div>
    </div>
  </div>
...
function bdGuardarContacto(){
  ...
  if(accion==='seguimiento'){
    ...
    // Crear alerta de venta para administración
    ...
    alertas.push({
      ...
      email:emp.email||'',
      tel:emp.telefono||'',
      cuit:document.getElementById('bdc-venta-cuit')?.value||'',
      rubro:document.getElementById('bdc-venta-rubro')?.value||emp.rubro||'',
      tipo:document.getElementById('bdc-venta-servicio').value,
      monto:Number(document.getElementById('bdc-venta-monto').value)||0,
      notas:nota||resultado,
      estado:'pendiente',
      segId:0,
    ...
    log.cerradas=(log.cerradas||0)+1;
    log.notas=(log.notas?log.notas+'; ':'')+'Venta: '+emp.empresa;
    S.set('crm_logs',logs);
    toast('🏆 ¡Venta registrada! Alerta enviada a Administración');
  } else {
    toast('✅ Contacto registrado');
  }
...
function madGetContext(){
  ...
  return `Sos MetoAsist, el asistente estrategico del dueno de MetoGroup, empresa de consultoria en certificaciones internacionales (BPC:2026, ISO 72001, adecuaciones IA). Tenes acceso completo a TODOS los datos del sistema. Tu rol es ser socio estrategico: analizas datos, detectas problemas, propones soluciones. Hablas en espanol rioplatense.

FECHA: ${today} | MES: ${mesNom} | DUENO: ${currentUser?.nombre||''}
...
${cuotasVenc.length?'Cuotas prox: '+cuotasVenc.slice(0,5).map(c=>c.cliente+' $'+c.monto+' en '+c.dias+'d').join('; '):''}
${cotsPend.length?'Cotizaciones pend: '+cotsPend.slice(0,3).map(c=>c.cliente+' $'+c.monto).join(', '):''}

COMERCIAL ${mesNom}: Llamadas ${totalLL} | Duenos ${totalDU} | Agendadas ${totalAG} | Cierres ${totalCI} | Conv ${convEq}%
${ventasPend.length?'VENTAS POR PROCESAR: '+ventasPend.map(v=>v.empresa+' $'+v.monto).join(', '):''}

VENDEDORES:
${ranking.map((r,i)=>(i+1)+'. '+r.nombre+': '+r.ci+'/'+r.objCierres+' cierres, '+r.ll+'/'+r.objLlam+' llam, '+r.ag+' agend, '+r.empresas+' emp('+r.interesadas+' inter), '+r.segPend+' seg pend').join('\n')}

BASE DATOS: ${bdStats.total} empresas | ${bdStats.pend} pend | ${bdStats.contact} contactadas | ${bdStats.inter} interesadas | ${totalContactos} contactos hechos

AUDITORIAS: ${audsActivas.length} activas | ${audsComp.length} completas | ${audsMes.length} nuevas mes | Total ${auds.length}
...
CONSULTORES: ${consDetalle.join(' | ')||'Ninguno'}

SEGUIMIENTOS: ${segsPend.length} pend | ${segsHoy.length} hoy | ${segsVenc.length} vencidos
${segsVenc.length?'Vencidos: '+segsVenc.slice(0,5).map(s=>s.empresa+'('+s.vendedor+')').join(', '):''}

CLIENTES: ${clientes.length} total | ${clientes.filter(c=>auds.find(a=>a.clienteId===c.id&&!['Completada','Informe Entregado'].includes(a.estado))).length} con aud activa
${objD?'OBJ DUENO: Auds:'+objD.auditorias+' IA:'+objD.ia+' Impl:'+objD.implementaciones+' Fact:$'+objD.facturacion+' Cobros:$'+objD.cobros:''}
${sueldos.length?'NOMINA: '+sueldos.length+' empleados':''}

REGLAS: Habla rioplatense. Sin asteriscos ni markdown. Se directo y estrategico. Max 8 oraciones salvo analisis extenso. Cita datos reales. Si detectas problemas mencionalo. Sos un par del dueno, opina y sugeri.`;
}
function madGetBriefingPrompt(){
  const ctx=madGetContext();
  return{
    system:ctx,
    messages:[{role:'user',content:`Dame el briefing ejecutivo del día. Quiero saber: 1) Situación financiera en una línea, 2) Alertas urgentes (vencimientos, seguimientos, cuotas), 3) Rendimiento comercial del equipo, 4) Una recomendación estratégica concreta. Formato: texto corrido, sin listas, sin asteriscos, máximo 8 oraciones. Hablame como un socio de confianza.`}]
  };
}
...
const BPC_DIMENSIONES = [
  ...
  {
    id:'D8', nombre:'Liderazgo Personal y Aprendizajes', maxPts:0, icon:'💡', color:'#f59e0b', esReflexiva:true,
    preguntas:[
      {id:'D8Q1', texto:'¿Sentís que tenés los conocimientos y las herramientas necesarias para tomar decisiones que hagan crecer el área comercial de tu empresa?', tipo:'reflexiva', ref:'A.5.9'},
      {id:'D8Q2', texto:'¿En qué aspectos de la gestión comercial sentís que te falta más formación o apoyo — estrategia, ventas, marketing, tecnología, liderazgo de equipos u otro?', tipo:'reflexiva', ref:'A.5.10'},
      {id:'D8Q3', texto:'¿Recordás alguna oportunidad de venta o de captación de un cliente que se haya perdido por falta de preparación, una respuesta lenta o un fallo de alguna persona o herramienta del equipo?', tipo:'reflexiva', ref:'A.8.1'},
      {id:'D8Q4', texto:'Si esa situación volviera a ocurrir hoy, ¿qué cambiaría en tu equipo, proceso o tecnología para que el resultado fuera distinto?', tipo:'reflexiva', ref:'A.8.2'},
      {id:'D8Q5', texto:'¿Qué tan abiertos creés que están tus clientes a que automatices partes del proceso comercial — seguimientos, recordatorios, propuestas, respuestas automáticas? ¿Lo verían como algo positivo o como una pérdida del trato personal?', tipo:'reflexiva', ref:'A.9.7'}
    ]
  }
];

const BPC_NIVELES = [
  {min:90, max:100, nombre:'Excelencia Comercial', color:'#c8a84a', icon:'🏆', desc:'La organización demuestra un sistema comercial maduro, integrado y en mejora continua.'},
  {min:75, max:89, nombre:'Madurez Avanzada', color:'#c8a84a', icon:'🌟', desc:'Sistema comercial sólido con oportunidades puntuales de optimización.'},
  {min:56, max:74, nombre:'Madurez Intermedia', color:'#f59e0b', icon:'📈', desc:'Bases establecidas con áreas significativas de mejora en procesos y sistemas.'},
  {min:36, max:55, nombre:'En Desarrollo', color:'#f97316', icon:'🔧', desc:'Procesos comerciales en construcción. Requiere trabajo estructural.'},
  {min:0, max:35, nombre:'Etapa Inicial', color:'#ef4444', icon:'🚨', desc:'Sistema comercial incipiente. Necesita intervención integral urgente.'}
];
...
async function agenteEnviarBienvenidaCliente(auditoriaId){
  ...
  const html = '<div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;padding:40px 32px;background:#ffffff;color:#1a1a1a">'
    ...
    +'<div style="font-family:Arial,sans-serif;font-size:10px;color:#999;letter-spacing:2px;text-transform:uppercase;margin-top:2px">Sistema de Gestión Comercial</div>'
    +'</div>'

    // Saludo
    +'<p style="font-size:15px;line-height:1.8;margin-bottom:20px">'+contacto+', buen día.</p>'

    // Cuerpo — voz de Hernán, humano
    +'<p style="font-size:15px;line-height:1.9;margin-bottom:18px">'
    +'Mi nombre es Hernán Quiroz, soy parte del equipo de MetoGroup y voy a estar acompañándolos durante todo este proceso. '
    +'Quería presentarme y contarles brevemente de qué se trata lo que arrancamos juntos.</p>'

    +'<p style="font-size:15px;line-height:1.9;margin-bottom:18px">'
    +'Con <strong>'+cliente.nombre+'</strong> iniciamos una <strong>Auditoría BPC:2026</strong> — que no es una certificación. '
    +'Lo que vamos a hacer es evaluar el <strong>grado de madurez comercial</strong> de la empresa: '
    +'cómo está el equipo de ventas, los procesos, la comunicación, el liderazgo comercial y la ética en cada punto de contacto. '
    +'El resultado es un informe concreto con un puntaje BPC Score y un plan de acción real para mejorar.</p>'

    +'<p style="font-size:15px;line-height:1.9;margin-bottom:28px">'
    +'El primer paso lo hace usted: es un diagnóstico inicial de 10 a 15 minutos dentro de nuestra plataforma. '
    +'No tiene respuestas correctas ni incorrectas — solo nos ayuda a entender el punto de partida antes de que arrancemos con el equipo.</p>'

    // Credenciales
    +'<div style="background:#fafaf7;border:1px solid #e0d5b0;border-radius:10px;padding:24px 28px;margin-bottom:32px">'
    +'<p style="font-family:Arial,sans-serif;font-size:12px;font-weight:700;color:#92770a;letter-spacing:1px;text-transform:uppercase;margin:0 0 16px 0">Sus datos de acceso</p>'
    +'<table style="font-size:14px;width:100%;border-collapse:collapse">'
    +'<tr><td style="padding:8px 0;color:#888;font-family:Arial,sans-serif;font-size:12px;width:110px;vertical-align:top">Plataforma</td>'
    +'<td style="padding:8px 0"><a href="'+urlSistema+'" style="color:#c8a84a;font-weight:600;font-family:Arial,sans-serif;font-size:13px">'+urlSistema+'</a></td></tr>'
    +'<tr><td style="padding:8px 0;color:#888;font-family:Arial,sans-serif;font-size:12px;vertical-align:top">Usuario</td>'
    +'<td style="padding:8px 0;font-family:Arial,sans-serif;font-weight:600;font-size:13px">'+cliente.email+'</td></tr>'
    +'<tr><td style="padding:8px 0;color:#888;font-family:Arial,sans-serif;font-size:12px;vertical-align:top">Contraseña</td>'
    +'<td style="padding:8px 0;font-family:Arial,sans-serif;font-weight:700;font-size:18px;letter-spacing:3px;color:#1a1a1a">'+passGenerada+'</td></tr>'
    +'</table>'
    +'</div>'

    // CTA
    +'<div style="text-align:center;margin:32px 0">'
    +'<a href="'+urlSistema+'" style="background:#c8a84a;color:#000000;padding:15px 40px;border-radius:6px;text-decoration:none;font-family:Arial,sans-serif;font-weight:700;font-size:14px;letter-spacing:0.5px;display:inline-block">Ingresar a la plataforma</a>'
    +'</div>'

    // Cierre humano
    +'<p style="font-size:15px;line-height:1.9;margin-bottom:8px">'
    +'Ante cualquier duda o inconveniente para ingresar, responda este email y lo resolvemos enseguida.</p>'
    +'<p style="font-size:15px;line-height:1.9;margin-bottom:32px">Quedo a su disposición.</p>'

    // Firma
    +'<div style="border-top:1px solid #eee;padding-top:24px">'
    +'<p style="font-size:14px;font-weight:700;margin:0 0 4px 0">'+agenteNombre+'</p>'
    +'<p style="font-family:Arial,sans-serif;font-size:12px;color:#888;margin:0 0 2px 0">MetoGroup Latam S.A.</p>'
    +'<a href="mailto:'+agenteEmail+'" style="font-family:Arial,sans-serif;font-size:12px;color:#c8a84a;text-decoration:none">'+agenteEmail+'</a>'
    +'</div>'
    +'</div>';
  ...
  try {
    ...
    if(idx>-1){ auds[idx].email_inicio_enviado = true; S.set('auditorias', auds); }
    console.log('Agente: Email bienvenida enviado a '+cliente.email);
  } catch(e){
    console.error('Agente: error enviando bienvenida', e);
  }
}
...
async function agenteEnviarAvanceProceso(auditoriaId){
  ...
  const html = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#ffffff">'
    +'<div style="font-size:22px;font-weight:700;margin-bottom:4px;color:#1a1a1a">MetoGroup</div>'
    +'<div style="font-size:10px;letter-spacing:2px;color:#888;text-transform:uppercase;margin-bottom:28px">Reporte de Avance — '+aud.clienteNombre+'</div>'
    +'<p style="font-size:15px;color:#1a1a1a;margin-bottom:16px">Hola <strong>'+nombreDest+'</strong>,</p>'
    +'<p style="font-size:13px;color:#444;line-height:1.7;margin-bottom:20px">Le enviamos un resumen del estado actual del proceso de auditoría BPC:2026 de <strong>'+aud.clienteNombre+'</strong>.</p>'
    +'<div style="background:#f8f8f8;border-radius:10px;padding:20px 24px;margin-bottom:24px">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">'
    +'<div style="font-size:13px;font-weight:700;color:#1a1a1a">Progreso general</div>'
    +'<div style="font-size:24px;font-weight:900;color:#c8a84a">'+pct+'%</div>'
    +'</div>'
    +'<div style="height:8px;background:#e5e5e5;border-radius:4px;margin-bottom:20px">'
    +'<div style="width:'+pct+'%;height:100%;background:#c8a84a;border-radius:4px"></div>'
    +'</div>'
    +pasos.map(p=>'<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'
      +'<div style="width:18px;height:18px;border-radius:50%;background:'+(p.ok?'#22c55e':'#e5e5e5')+';display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:'+(p.ok?'#fff':'#999')+';flex-shrink:0">'+(p.ok?'✓':'○')+'</div>'
      +'<div style="font-size:13px;color:'+(p.ok?'#1a1a1a':'#888')+'">'+(p.ok?'<strong>':'')+(p.label)+(p.ok?'</strong>':'')+'</div>'
      +'</div>'
    ).join('')
    +'</div>'
    +'<div style="background:#fffbea;border-left:4px solid #c8a84a;padding:16px 20px;border-radius:0 8px 8px 0;margin-bottom:24px">'
    +'<div style="font-size:13px;font-weight:700;color:#92770a;margin-bottom:8px">📅 Próximas fechas</div>'
    +(aud.fDoc && aud.fDoc >= hoy?'<div style="font-size:13px;color:#555;margin-bottom:4px">Auditoría documental: <strong>'+fmtFecha(aud.fDoc)+'</strong></div>':'')
    +(aud.fInsitu && aud.fInsitu >= hoy?'<div style="font-size:13px;color:#555;margin-bottom:4px">Auditoría in situ: <strong>'+fmtFecha(aud.fInsitu)+'</strong></div>':'')
    +(aud.fExterna && aud.fExterna >= hoy?'<div style="font-size:13px;color:#555">Aud. comunicaciones: <strong>'+fmtFecha(aud.fExterna)+'</strong></div>':'')
    +'</div>'
    +'<p style="font-size:12px;color:#888;line-height:1.6;margin-bottom:24px">Ante cualquier consulta o necesidad de reprogramar alguna fecha, responda este email.</p>'
    +'<div style="border-top:1px solid #eee;padding-top:20px;font-size:11px;color:#888;line-height:1.8">'
    +fromName+'<br>'
    +'<a href="mailto:'+fromEmail+'" style="color:#c8a84a">'+fromEmail+'</a><br>'
    +'MetoGroup Latam S.A.'
  ...
  try {
    await fetch('/.netlify/functions/send-email',{
      ...
      body: JSON.stringify({
        ...
        subject: '📊 Avance de auditoría BPC:2026 — '+pct+'% completado | '+aud.clienteNombre,
        html,
        text: 'Reporte de avance auditoría '+aud.clienteNombre+': '+pct+'% completado. Próximas fechas: Doc '+fmtFecha(aud.fDoc)+' | In situ '+fmtFecha(aud.fInsitu)+'.'
      })
    });
    // Guardar fecha del último avance enviado
    const auds = S.get('auditorias');
    const idx = auds.findIndex(x=>x.id===auditoriaId);
    if(idx>-1){ auds[idx].ultimo_avance_enviado = hoy; S.set('auditorias', auds); }
    console.log('🤖 Agente: Avance enviado a '+destinatario);
  } catch(e){
    console.error('Agente: error enviando avance', e);
  }
}
...
async function agenteCoordinarExamenes(auditoriaId){
  ...
  const html = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#ffffff">'
    +'<div style="font-size:22px;font-weight:700;margin-bottom:4px;color:#1a1a1a">MetoGroup</div>'
    +'<div style="font-size:10px;letter-spacing:2px;color:#888;text-transform:uppercase;margin-bottom:28px">Auditoría BPC:2026</div>'
    +'<p style="font-size:15px;color:#1a1a1a;margin-bottom:16px">Hola <strong>'+encargadoNombre+'</strong>,</p>'
    +'<p style="font-size:13px;color:#444;line-height:1.7;margin-bottom:16px">'
    +'Soy el sistema de coordinación de MetoGroup para la auditoría BPC:2026 de <strong>'+cliente.nombre+'</strong>. '
    +'El diagnóstico inicial ya fue completado. El próximo paso son los <strong>exámenes individuales del equipo comercial</strong>, '
    +'programados para la semana del <strong>'+fechaExamFmt+'</strong>.</p>'
    +'<div style="background:#fffbea;border-left:4px solid #c8a84a;padding:18px 20px;border-radius:0 8px 8px 0;margin-bottom:24px">'
    +'<div style="font-weight:700;font-size:13px;color:#92770a;margin-bottom:10px">📋 Necesitamos los datos de:</div>'
    +'<ul style="font-size:13px;color:#555;line-height:2;margin:0;padding-left:18px">'
    +'<li><strong>Vendedores</strong> — nombre completo y email de cada uno</li>'
    +'<li><strong>Gerente comercial</strong> — nombre y email</li>'
    +'<li><strong>Dueño / Director</strong> — confirmar si el dueño ya registrado participa del examen</li>'
    +'</ul>'
    +'</div>'
    +'<p style="font-size:13px;color:#444;line-height:1.7;margin-bottom:8px">'
    +'Una vez que nos envíe esos datos, el sistema generará automáticamente los accesos individuales y enviará a cada persona su link de examen por email. '
    +'<strong>Cada examen toma entre 20 y 30 minutos</strong> y puede realizarse desde cualquier dispositivo.</p>'
    +'<p style="font-size:13px;color:#444;line-height:1.7;margin-bottom:24px">'
    +'Por favor responda este email con el listado del equipo antes del <strong>'+fechaExamFmt+'</strong>.</p>'
    +'<div style="border-top:1px solid #eee;padding-top:20px;font-size:11px;color:#888;line-height:1.8">'
    +fromName+'<br>'
    +'<a href="mailto:'+fromEmail+'" style="color:#c8a84a">'+fromEmail+'</a><br>'
    +'MetoGroup Latam S.A.'
  ...
  try {
    await fetch('/.netlify/functions/send-email',{
      ...
      body: JSON.stringify({
        ...
        from_name: fromName,
        to: encargadoEmail,
        subject: '📋 Datos del equipo para exámenes BPC:2026 — '+cliente.nombre+' (semana del '+fechaExamFmt+')',
        html,
        text: 'Necesitamos los datos (nombre + email) de: vendedores, gerente y dueño para enviarles sus links de examen BPC:2026. Fecha estimada: '+fechaExamFmt+'.'
      })
    });
    // Marcar que se solicitaron los datos de exámenes
    const auds = S.get('auditorias');
    const idx = auds.findIndex(x=>x.id===auditoriaId);
    if(idx>-1){ auds[idx].examenes_solicitados = true; auds[idx].fExamenesEstimada = fechaExam; S.set('auditorias', auds); }
    console.log('🤖 Agente: Solicitud de exámenes enviada a '+encargadoEmail);
  } catch(e){
    console.error('Agente: error coordinando examenes', e);
  }
}
...
async function agenteEnviarCodigosExamen(auditoriaId, equipo){
  ...
  for(const persona of equipo){
    ...
    const rolTipo = persona.rol === 'gerente' ? 'diagnostico_gerente'
                  ...
                  : 'auditoria';
    const _tipoExamen = persona.rol === 'gerente' ? 'gerente' : 'vendedor';
    const examUrl = 'https://taupe-raindrop-c74337.netlify.app/?examen_token=' + codigo
                  + '&tipo=' + _tipoExamen
                  + '&empresa=' + encodeURIComponent(cliente.nombre)
    ...
    const html = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#ffffff">'
      +'<div style="font-size:22px;font-weight:700;margin-bottom:4px;color:#1a1a1a">MetoGroup</div>'
      +'<div style="font-size:10px;letter-spacing:2px;color:#888;text-transform:uppercase;margin-bottom:28px">Auditoría BPC:2026 · '+cliente.nombre+'</div>'
      +'<p style="font-size:15px;color:#1a1a1a;margin-bottom:16px">Hola <strong>'+persona.nombre+'</strong>,</p>'
      +'<p style="font-size:13px;color:#444;line-height:1.7;margin-bottom:20px">'
      +'Tu empresa <strong>'+cliente.nombre+'</strong> está realizando una auditoría de Buenas Prácticas Comerciales y Éticas (BPC:2026). '
      +'Como parte del proceso, te pedimos que completes el siguiente cuestionario individual. '
      +'Tus respuestas son <strong>confidenciales</strong> y no serán compartidas con tu empresa — solo el puntaje general.</p>'
      +'<div style="background:#fffbea;border-left:4px solid #c8a84a;padding:20px 24px;border-radius:0 8px 8px 0;margin-bottom:28px;text-align:center">'
      +'<div style="font-size:11px;color:#92770a;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px">Tu código de acceso</div>'
      +'<div style="font-family:monospace;font-size:36px;font-weight:900;color:#c8a84a;letter-spacing:8px;margin-bottom:8px">'+codigo+'</div>'
      +'<div style="font-size:12px;color:#888">'+rolLabel+' · '+duracion+'</div>'
      +'</div>'
      +'<div style="text-align:center;margin-bottom:28px">'
      +'<a href="'+examUrl+'" style="background:#c8a84a;color:#000;padding:14px 32px;border-radius:6px;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:0.5px;display:inline-block">▶ COMENZAR CUESTIONARIO</a>'
      +'</div>'
      +'<p style="font-size:12px;color:#888;line-height:1.6;margin-bottom:24px">'
      +'Podés hacer el cuestionario desde cualquier dispositivo. Si cerrás la página antes de terminar, tu código te permite retomar desde donde dejaste. '
      +'Cualquier duda respondé este email.</p>'
      +'<div style="border-top:1px solid #eee;padding-top:20px;font-size:11px;color:#888;line-height:1.8">'
      +fromName+'<br>'
      +'<a href="mailto:'+fromEmail+'" style="color:#c8a84a">'+fromEmail+'</a><br>'
      +'MetoGroup Latam S.A.'
    ...
    try {
      await fetch('/.netlify/functions/send-email',{
        ...
        body: JSON.stringify({
          ...
          html,
          text: 'Hola '+persona.nombre+', tu código para el '+rolLabel+' es: '+codigo+'. Accedé en: '+examUrl
        })
      });
      enviados.push(persona.nombre);
      await new Promise(r=>setTimeout(r,1500));
    } catch(e){
      console.error('Agente: error enviando código a '+persona.email, e);
    }
  }
...
function _portalFinalizar(){
  ...
  // ── Actualizar auditoría vinculada en caché + Supabase ──
  const auds = S.get('auditorias')||[];
  const aud = auds.find(a=>String(a.clienteId)===String(currentUser.clienteId));
  if(aud){
    aud.diagnostico_ok=true; aud.diagnostico_score=diag.score; aud.diagnostico_fecha=todayStr();
    if(aud.estado==='Pendiente'||aud.estado==='Iniciada') aud.estado='En proceso';
    const ai=auds.findIndex(a=>a.id===aud.id); if(ai>-1) auds[ai]=aud;
    _sbCache['auditorias']=JSON.parse(JSON.stringify(auds));
    try{ localStorage.setItem('METO_auditorias',JSON.stringify(auds)); }catch(e){}
    sbFetch('auditorias','PATCH',{diagnostico_ok:true,diagnostico_score:diag.score,diagnostico_fecha:todayStr(),estado:aud.estado},'?id=eq.'+aud.id)
      .then(r=>console.log('✅ _portalFinalizar auditoría PATCH OK',r))
      .catch(e=>console.error('❌ _portalFinalizar auditoría PATCH error:',e));
  } else {
    console.warn('⚠️ _portalFinalizar — sin auditoría en caché para clienteId='+currentUser.clienteId);
    sbFetch('auditorias','GET',null,'?clienteId=eq.'+currentUser.clienteId+'&select=id,estado').then(rows=>{
      if(rows&&rows.length){
        sbFetch('auditorias','PATCH',{diagnostico_ok:true,diagnostico_score:diag.score,diagnostico_fecha:todayStr()},'?id=eq.'+rows[0].id);
      }
    });
  }
...
function _portalRenderAgradecimiento(scene, diag){
  ...
  const C = isLight ? {
    bg:'rgba(255,255,255,0.06)', bdr:'rgba(212,175,55,0.25)',
    txt:'rgba(255,255,255,0.92)', txt2:'rgba(255,255,255,0.62)', txt3:'rgba(255,255,255,0.38)',
    scorec:'#d4af37',
  } : {
    bg:'rgba(212,175,55,0.04)', bdr:'rgba(212,175,55,0.18)',
    txt:'rgba(255,255,255,0.88)', txt2:'rgba(255,255,255,0.45)', txt3:'rgba(255,255,255,0.22)',
    scorec:'#d4af37',
  };

  scene.innerHTML =
    '<div style="width:100%;max-width:620px;padding:clamp(48px,8vh,88px) clamp(24px,5vw,52px);display:flex;flex-direction:column;animation:_swIn 0.85s cubic-bezier(0.16,1,0.3,1) both">'

      // MetoAsist header
      +'<div style="display:flex;align-items:center;gap:14px;margin-bottom:40px">'
        +'<div style="width:46px;height:46px;border-radius:50%;flex-shrink:0;background:radial-gradient(circle at 35% 30%,#f5d060,#d4af37 50%,#8a6a1a);display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 0 28px rgba(212,175,55,0.28);animation:orbPulse 3s ease-in-out infinite">✦</div>'
        +'<div style="flex:1">'
          +'<div style="font-family:\'Space Mono\',monospace;font-size:13px;font-weight:700;color:#d4af37;letter-spacing:0.05em">MetoAsist</div>'
          +'<div style="font-family:\'Space Mono\',monospace;font-size:9px;letter-spacing:0.18em;text-transform:uppercase;color:'+C.txt3+';margin-top:3px">Diagnóstico completado</div>'
        +'</div>'
        +'<div style="display:flex;align-items:center;gap:6px">'
          +'<div style="width:5px;height:5px;border-radius:50%;background:#d4af37;box-shadow:0 0 6px #d4af37;animation:glow-pulse 2s ease-in-out infinite"></div>'
          +'<span style="font-family:\'Space Mono\',monospace;font-size:7px;letter-spacing:0.15em;color:rgba(212,175,55,0.45);text-transform:uppercase">Procesando</span>'
        +'</div>'
      +'</div>'

      +'<div style="height:1px;background:linear-gradient(90deg,rgba(212,175,55,0.3),transparent);margin-bottom:44px"></div>'

      // Saludo y gracias
      +'<div style="font-family:\'Space Mono\',monospace;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:'+C.txt3+';margin-bottom:10px">Excelente trabajo</div>'
      +'<div style="font-family:\'Cormorant Garamond\',serif;font-size:clamp(40px,6vw,66px);font-weight:300;line-height:0.95;letter-spacing:-0.03em;color:'+C.txt+';margin-bottom:36px">'+(nombre?nombre+'.':'¡Listo!')+'</div>'


      // Mensaje principal
      +'<div style="border-left:2px solid rgba(212,175,55,0.25);padding-left:22px;display:flex;flex-direction:column;gap:18px;margin-bottom:48px">'
        +'<p style="font-family:\'Cormorant Garamond\',serif;font-size:clamp(17px,2.3vw,21px);font-weight:300;font-style:italic;line-height:1.75;color:'+C.txt2+';margin:0">'
          +'Gracias por completar el diagnóstico inicial BPC:2026. Tus respuestas nos dan una foto muy clara del punto de partida comercial de la organización.'
        +'</p>'
        +'<p style="font-family:\'Cormorant Garamond\',serif;font-size:15px;font-weight:300;line-height:1.85;color:'+C.txt2+';margin:0">'
          +'En las próximas <strong style="font-weight:600;color:'+C.scorec+';font-style:italic">24 horas</strong> el equipo de MetoGroup va a configurar tu dashboard de auditoría personalizado con el análisis completo de resultados, las dimensiones prioritarias y el plan de acción sugerido.'
        +'</p>'
        +'<p style="font-family:\'Space Mono\',monospace;font-size:9px;letter-spacing:0.08em;line-height:1.9;color:'+C.txt3+';margin:0">'
          +'Recibirás una notificación por email cuando esté listo. Si tenés alguna consulta antes de eso, escribinos usando el ícono ✦ en la barra superior.'
        +'</p>'
      +'</div>'

      +'<div style="font-family:\'Space Mono\',monospace;font-size:10px;letter-spacing:0.15em;color:'+C.txt3+';text-transform:uppercase">MetoGroup Latam S.A. · BPC:2026</div>'
...
function calcDimScore(diag,dim){
  ...
  const maxRaw=dim.preguntas.length*5;
  const rawScore=resps.reduce((s,v)=>s+v,0);
  return Math.round(rawScore/maxRaw*dim.maxPts);
}
